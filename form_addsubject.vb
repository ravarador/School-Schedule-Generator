    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If (TextBox1.Text <> "") Then
            Try
                ...
                adapter.Fill(table)
                form_viewdatabase.datagridview_subject.DataSource = table
                ...
                Try
                    ...
                    sqlite_cmd.CommandText = "INSERT INTO Subject_list (subject_name) VALUES ('" & TextBox1.Text & "');"
                    sqlite_cmd.ExecuteNonQuery()
                    sqlite_conn.Close()

                    TextBox1.Text = ""
                Catch ex As Exception
                    MessageBox.Show("Subject name already exists!")
                End Try

                'UPDATE THE DATAGRID VIEW
                Dim adapter3 As New SQLiteDataAdapter("SELECT * FROM Subject_list", connectionString)
                Dim table3 As New DataTable()
                adapter.Fill(table3)
                form_viewdatabase.datagridview_subject.DataSource = table3

                'FILL THE LISTBOX WITH DATAGRID ITEMS
                Dim adapter1 As New SQLiteDataAdapter("SELECT * FROM Subject_list", connectionString)
                Dim table1 As New DataTable()
                adapter.Fill(table1)
                form_main.listbox_subject.DisplayMember = "subject_name"
                form_main.listbox_subject.DataSource = table1
